...
![变量的测试（是否存在、是否为空）和内容替换](img/var_test.png)

`${var:-string},${var:=string},${var:+string},${var:?string}`
+ 1.`${var:-string}`和`${var:=string}`:若变量var为空，则用在命令行中用string来替换`${var:-string}`，否则变量var不为空时，则用变量var的值来替换`${var:-string}`；对于`${var:=string}`的替换规则和`${var:-string}`是一样的，所不同之处是`${var:=string}`若var为空时，用string替换`${var:=string}`的同时，把string赋给变量`var： ${var:=string}`很常用的一种用法是，判断某个变量是否赋值，没有的话则给它赋上一个默认值。
...
# 从左边开始，删除最短匹配r, 这里的*是通配符,表示零个或多个任意字符,？表示仅与一个任意字符匹配
echo ${var#*r}        ==> 1/dir2/file.txt
# 从左边开始，删除最短匹配dir
echo ${var#*dir}      ==> 1/dir2/file.txt
# 从左边开始，删除最短匹配数字，支持正则表达式
echo ${var#*[0-9]}    ==> /dir2/file.txt
# 从左边开始，删除最长匹配di，这里的*表示删除
echo ${var##*di}      ==> r2/file.txt
# 从右边开始，删除最短匹配di，注意：这里的*在后面。 #在$的左边，从左边开始，%在$的右边，从右边开始
echo ${var%di*}       ==> /dir1/
# 从右边开始，删除最长匹配di
echo ${var%%di*}     ==> /
# 用aa替换第一个匹配的di
echo ${var/di/aa}     ==> /aar1/dir2/file.txt
# 用bbb替换所有匹配的dir
echo ${var//dir/bbb}  ==> /bbb1/bbb2/file.txt
# 用bbb替换前缀/dir,注意:/本身需要使用转义字符
echo ${var/#\/dir/bbb}  ==> bbb1/dir2/file.txt
# 用bbb替换前缀le,注意:/本身需要使用转义字符
 echo ${var/%\le*.txt/bbb}  ==> /dir1/dir2/fibbb
# 用bbb替换后缀2*.txt,支持正则表达式
echo ${var/%2*.txt/bbb}     ==> /dir1/dirbbb
```
